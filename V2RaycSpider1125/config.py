...
if "win" in platform:
    ...
    SERVER_DIR_PROJECT = env.str("SERVER_DIR_PROJECT", f"/qinse/V2RaycSpider{verNum}")
...
# 版本管理文件
SERVER_PATH_DEPOT_VCS = env.str(
    "SERVER_PATH_DEPOT_VCS", join(SERVER_DIR_CLIENT_DEPORT, "vcs.csv")
)

# 服务器日志文件路径<业务层>
SERVER_DIR_DATABASE_LOG = env.str(
    "SERVER_DIR_DATABASE_LOG", join(SERVER_DIR_DATABASE, "logs")
)
logger.add(
    env.str("LOG_RUNTIME_FILE", join(SERVER_DIR_DATABASE_LOG, "runtime.log")),
    level="DEBUG",
    rotation="1 week",
    retention="20 days",
    encoding="utf8",
...
logger.add(
    env.str("LOG_ERROR_FILE", join(SERVER_DIR_DATABASE_LOG, "error.log")),
    level="ERROR",
    rotation="1 week",
    encoding="utf8",
)
...
# 软件本地根目录
ROOT_DIR_PROJECT = (
    env.str("ROOT_PROJECT_PATH", "C:\\V2RaySpider")
    if "win" in platform
    else env.str("ROOT_PROJECT_PATH", dirname(__file__))
)

# 软件本地日志目录
LOCAL_DIR_LOG = env.str("LOCAL_DIR_LOG", join(ROOT_DIR_PROJECT, "logs"))

# 本地运行日志
logger_local.add(
    env.str(
        "LOG_RUNTIME_FILE_LOCAL",
...
logger_local.add(
    env.str("LOG_ERROR_FILE_LOCAL", join(LOCAL_DIR_LOG, "error_local.log")),
    level="ERROR",
    rotation="1 week",
    encoding="utf8",
)
...
# 软件本地数据仓库
LOCAL_DIR_DATABASE = env.str("LOCAL_DIR_FETCH", join(ROOT_DIR_PROJECT, "Database"))

# 软件本地请求历史
LOCAL_PATH_DATABASE_FH = env.str(
    "LOCAL_PATH_DATABASE_FH", join(LOCAL_DIR_DATABASE, "FetchRequestsHistory.txt")
)

# 软件查看机场生态的缓存文件
LOCAL_PATH_AIRPORT_INFO = env.str(
    "LOCAL_PATH_AIRPORT_INFO", join(LOCAL_DIR_DATABASE, "FetchAirEcologyInfo.csv")
)

# 软件本地的客户端配置文件
LOCAL_PATH_DATABASE_YAML = env.str(
...
# 软件默认下载仓库
LOCAL_DIR_DEPOT = env.str("LOCAL_DIR_DEPOT", join(ROOT_DIR_PROJECT, "depot"))

# 软件更新模组
PLUGIN_UPDATED_MODULE = env.str(
    "PLUGIN_UPDATED_MODULE", join(LOCAL_DIR_DEPOT, "updated.exe")
)

# 各版本v2raycs默认下载路径
LOCAL_DIR_DEPOT_CLIENT = env.str(
