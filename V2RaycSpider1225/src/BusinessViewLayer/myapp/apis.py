...
def apis_capture_subscribe(style: dict) -> dict:
    ...
    if style.get("type") in CRAWLER_SEQUENCE:
        ...
        try:
            with open(
                NGINX_SUBSCRIBE.format(style.get("type")), "r", encoding="utf-8"
            ) as f:
...
def apis_version_manager(
    ...
    try:
        ...
        if header:
            response.update({"version-server": data[1:][-1][0], "url": data[1:][-1][1]})
        else:
            response.update({"version-server": data[-1][0], "url": data[-1][1]})
...
def apis_admin_get_entropy() -> list:
    return (
        RedisClient()
        .get_driver()
        .get(REDIS_SECRET_KEY.format("__entropy__"))
        .split("$")
    )
