...
class ServiceDiscovery(ActionMasterGeneral):
    def __init__(self, register_url, subs_type: str = None, silence=True, output_path_csv: str = None):
        """
        ...
        """
        super(ServiceDiscovery, self).__init__(silence=silence, assault=True, endurance=False)
        ...
        # =====================================================
        self.report_title = ["provider", "subType", "passable", "enDay", "enFlow", "nodeNum", "context"]
        ...
        if output_path_csv is None:
            ...
            self.output_path_csv = os.path.join(SERVER_DIR_DATABASE_CACHE, output_path_csv)
        else:
    ...
    def get_share_link(self, api=None):
        logger.info("<ServiceDiscovery> silence={} assault={} endurance={} url={}".format(
            self.silence, self.assault, self.endurance, self.register_url,
        ))
        ...
        try:
            ...
            logger.error("<ServiceDiscovery> The task strategy does not match, "
    ...
    def get_runtime_report(self, share_type="v2ray") -> dict:
        ...
        if nodes:
            try:
                ...
                details['enDay'] = [i.get("remark") for i in nodes if "时间" in i.get("remark")][0]
                details['enFlow'] = [i.get("remark") for i in nodes if "流量" in i.get("remark")][0]
            except IndexError:
                pass
        return details
