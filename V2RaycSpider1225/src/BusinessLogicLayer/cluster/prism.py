...
class Prism(ActionMasterGeneral):
    def __init__(self, atomic: dict, silence=True, assault=True):
        ...
        self.register_url = "" if atomic.get(
            "register_url") is None else atomic.get("register_url")
        self.hyper_params = {} if atomic.get(
            "hyper_params") is None else atomic.get("hyper_params")
        self.action_name = "PrismInstance" if atomic.get(
            "name") is None else atomic.get("name")
        self.life_cycle = 1 if atomic.get(
            "life_cycle") is None else atomic.get("life_cycle")
        self.anti_slider = True if atomic.get(
            "anti_slider") is None else atomic.get("anti_slider")
        self.atomic = atomic
...
class ServiceDiscovery(ActionMasterGeneral):
    def __init__(self,
        ...
        """
        super(ServiceDiscovery, self).__init__(silence=silence,
                                               assault=True,
                                               endurance=False)
        ...
        # =====================================================
        self.report_title = [
            "provider", "subType", "passable", "enDay", "enFlow", "nodeNum",
            "context"
        ]
        ...
        if output_path_csv is None:
            ...
            self.output_path_csv = os.path.join(SERVER_DIR_DATABASE_CACHE,
                                                output_path_csv)
        else:
    ...
    def get_share_link(self, api=None):
        logger.info(
            "<ServiceDiscovery> silence={} assault={} endurance={} url={}".
            format(
                self.silence,
                self.assault,
                self.endurance,
        ...
        try:
            ...
            logger.error(
                f"<ServiceDiscovery> TimeoutException url={self.register_url}")
        except UnexpectedAlertPresentException as e:
            logger.error(
                f"<ServiceDiscovery> {e.alert_text} url={self.register_url}")
        finally:
            api.quit()
    ...
    def get_runtime_report(self, share_type="v2ray") -> dict:
        ...
        result = SubscribeParser(
            self.subscribe).parse_subscribe(auto_base64=True)

        # Convert data type: strField --> Dict
        nodes = [
            dict(json.loads(node)) for node in reversed(result.get("nodes"))
        ]
