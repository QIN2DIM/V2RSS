...
class SpawnBooster(CoroutineSpeedup):
    def __init__(self,
        ...
        # 将单例视为容量为1的sequence
        # 允许sequence中出现重复的任务
        self.docker = docker if isinstance(docker, list) else [
            docker,
        ]
        self.jobs = []
    ...
    def offload_task(self):
        ...
        for mirror_image in self.docker:
            ...
            if mirror_image.get("feature") == 'prism':
                entity_ = Prism(atomic=mirror_image,
                                silence=self.silence,
                                assault=self.assault).run
            else:
                entity_ = ActionShunt.generate_entity(atomic=mirror_image,
                                                      silence=self.silence,
...
def booster(docker: dict or list,
            silence: bool,
            power: int = 1,
            assault=False):
    """
    ...
    if isinstance(docker, dict):
        # 对该实体发起一次行为测试
        if power == 1:
            ActionShunt.generate_entity(atomic=docker,
                                        silence=silence,
                                        assault=assault)()
    ...
    # 此时认为docker_list.__len__()>=2，否则将会被识别为单个实体分流至其他业务模块
        ...
        if power == 1:
            for mirror_image in docker:
                ActionShunt.generate_entity(atomic=mirror_image,
                                            silence=silence,
                                            assault=assault)()
        ...
        # 建立协程空间，分发实体行为测试任务[use gevent]
            ...
            SpawnBooster(docker=docker, silence=silence,
                         assault=assault).run(power)
