...
class _ClashAdaptationInterface:
    ...
    def __init__(self, subscribe: list = None, debug: bool = False):
        ...
        self.LOCAL_CONFIG_PATH = os.path.join(SERVER_DIR_DATABASE_CACHE,
                                              'clash_config_temple.yaml')
        self.CLASH_CONFIG_YAML = os.path.join(SERVER_DIR_DATABASE_CACHE,
                                              'clash_config.yaml')
    ...
    def _load_startup_config(self, url: str, path: str) -> dict:
        try:
            raw = requests.get(url, timeout=5,
                               proxies=self._PROXIES).content.decode('utf-8')
    ...
    def _v2ray_to_yaml(self, arr: list) -> dict:
        ...
        for item in arr:
            ...
            obj = {
                ...
                'ws-headers': {
                    'Host': item.get('host')
                } if item.get('host') else None
            }
            for key in list(obj.keys()):
                if obj.get(key) is None:
                    del obj[key]
            if obj.get('alterId') is not None and not obj['name'].startswith(
                    '剩余流量') and not obj['name'].startswith('过期时间'):
    ...
    def _ssr_to_yaml(self, arr: list) -> dict:
        ...
        for item in arr:
            obj = {
                'name':
                item.get('remarks').strip() if item.get('remarks') else None,
                'type': 'ssr',
            ...
            for key in list(obj.keys()):
                if obj.get(key) is None:
                    del obj[key]
            if (obj.get('name') and not obj['name'].startswith('剩余流量')
    ...
    def _generate_yaml(self, cache_path: str, config_content: dict) -> None:
        clash_yaml = yaml.dump(data=config_content,
                               sort_keys=False,
    ...
    def _analyze_ssr(self, nodes: list) -> list:
        ...
        for node in nodes:
            ...
            info['password'] = self.url_decode(
                password_params[0]).decode('utf-8')
            params = password_params[1].split('&')
            for p in params:
                key_value = p.split('=')
                info[key_value[0]] = self.url_decode(
                    key_value[1]).decode('utf-8')
    ...
    def _analyze_rss(self) -> dict:
        ...
        proxy_list = {'proxy_list': [], 'proxy_names': []}
        for run_entity in self._rss_pool:
            self._debug_printer(run_entity)
            res = requests.get(url=run_entity,
                               headers=headers,
                               timeout=5,
    ...
    def run(self) -> None:
        ...
        # 获取Clash配置写法模板
        default_config = self._load_startup_config(url=self.DEFAULT_CONFIG_URL,
                                                   path=self.LOCAL_CONFIG_PATH)
        response_config = self._generate_model(rss_pool, default_config)

        # 生成并缓存模板文件
        self._generate_yaml(cache_path=self.CLASH_CONFIG_YAML,
                            config_content=response_config)
...
class _Interface:
    ...
    def url_scheme_download() -> dict:
        return {
            "msg": "success",
            "info": _ClashAdaptationInterface().url_scheme(mode='download')
        }
    ...
    def url_scheme_quick() -> dict:
        return {
            "msg": "success",
            "info": _ClashAdaptationInterface().url_scheme(mode='quick')
        }
