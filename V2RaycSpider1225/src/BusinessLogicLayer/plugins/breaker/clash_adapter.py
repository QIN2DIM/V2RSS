...
class _ClashAdaptationInterface:
    ...
    def __init__(self, subscribe: list = None, debug: bool = False):
        ...
        self.DEFAULT_CONFIG_URL = (
            "https://cdn.jsdelivr.net/gh/Celeter/convert2clash@main/config.yaml"
        )
        self.LOCAL_CONFIG_PATH = os.path.join(
            SERVER_DIR_DATABASE_CACHE, "clash_config_temple.yaml"
        )
        self.CLASH_CONFIG_YAML = os.path.join(
            SERVER_DIR_DATABASE_CACHE, "clash_config.yaml"
        )
    ...
    def _load_startup_config(self, url: str, path: str) -> dict:
        try:
            raw = requests.get(url, timeout=5, proxies=self._PROXIES).content.decode(
                "utf-8"
            )
    ...
    def _v2ray_to_yaml(self, arr: list) -> dict:
        ...
        for item in arr:
            if (
                item.get("ps") is None
                and item.get("add") is None
                and item.get("port") is None
                and item.get("id") is None
                and item.get("aid") is None
            ):
                continue
            obj = {
    ...
    def _analyze_rss(self) -> dict:
        ...
        for run_entity in self._rss_pool:
            ...
            res = requests.get(
                url=run_entity, headers=headers, timeout=5, proxies=self._PROXIES
            )
            try:
    ...
    def run(self) -> None:
        ...
        # 获取Clash配置写法模板
        default_config = self._load_startup_config(
            url=self.DEFAULT_CONFIG_URL, path=self.LOCAL_CONFIG_PATH
        )
        response_config = self._generate_model(rss_pool, default_config)

        # 生成并缓存模板文件
        self._generate_yaml(
            cache_path=self.CLASH_CONFIG_YAML, config_content=response_config
        )
