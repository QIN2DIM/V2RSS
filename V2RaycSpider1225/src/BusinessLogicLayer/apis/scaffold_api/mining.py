...
from BusinessCentralLayer.setting import (
    ...
    SERVER_DIR_OUTPUT_STORE_CLASSIFIER,
    CHROMEDRIVER_PATH,
    TIME_ZONE_CN,
    logger,
...
from BusinessLogicLayer.utils.sspanel_mining import SSPanelHostsCollector
...
PATH_DATASET_TEMPLATE = os.path.join(
    SERVER_DIR_STORE_COLLECTOR, _FOCUS_PREFIX + "_{}" + _FOCUS_SUFFIX
)
...
def load_sspanel_hosts() -> list:
    ...
    # 识别并读回 Collector 输出
    for t in os.listdir(SERVER_DIR_STORE_COLLECTOR):
        if t.endswith(_FOCUS_SUFFIX) and t.startswith(_FOCUS_PREFIX):
            # 补全路径模版
            path_file_txt = os.path.join(SERVER_DIR_STORE_COLLECTOR, t)
            # 读回 Collector 输出
            with open(path_file_txt, "r", encoding="utf8") as f:
...
def load_classified_hosts(filter_: bool = True) -> list:
    ...
    # 读取 mining 分类结果
    classifier_outputs = [
        os.path.join(SERVER_DIR_OUTPUT_STORE_CLASSIFIER, i)
        for i in os.listdir(SERVER_DIR_OUTPUT_STORE_CLASSIFIER)
    ...
    if not classifier_outputs:
        logger.critical("默认目录下缺少分类器的缓存文件 - "
                        f"dir={SERVER_DIR_OUTPUT_STORE_CLASSIFIER}")
        sys.exit()
...
def run_classifier(power: int = 16, source: str = "local", batch: int = 1):
    ...
    path_output = output_cleaning_dataset(
        dir_output=SERVER_DIR_OUTPUT_STORE_CLASSIFIER,
        docker=docker,
    )
