...
class _ClashTaskAsh:
    ...
    def run_business(self):
        ...
        rss_pool = [
            ...
            for subscribe, _ in rc.hgetall(REDIS_SECRET_KEY.format(key_)).items()
        ]
        # 2.1 筛选订阅防止重名
        rss_dict = {}
        for url in rss_pool:
        ...
        # 5.创建本地连接 启动Clash
        webbrowser.open(
            clash_adapter.api.url_scheme_download()["info"].format(
    ...
    def startup(self):
        ...
        try:
            ...
            process_server = multiprocessing.Process(
                target=self.run_server, name="VirtualStation"
            )
            # 执行业务
