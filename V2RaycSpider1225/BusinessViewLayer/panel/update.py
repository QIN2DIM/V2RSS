...
from BusinessViewLayer.panel.config_panel import *
    ...
    def run(self, use_updated=False):
        try:
            ...
            if usr_choice:
                ...
                if use_updated is False:
                    ...
                    for x in range(3):
                        self.open_dir = easygui.diropenbox(
                            "请选择安装路径", install_title, default=LOCAL_DIR_DATABASE
                        )
                        # 退出-放弃更新
                ...
                # 自启动
                target_file = (
                        self.open_fp.replace(".zip", "") + f"_v{get_server_version()[0]}"
                )
                try:
                    ...
                    for filename in os.listdir(self.open_dir):
                        if ".zip" in filename:
                            try:
                                ...
                                os.path.basename(target_file).split("_")[-1]
                                != filename.split("_")[-1]
                        ):
...
class UnZipManager(object):
    ...
    def unzip(self, filename: str):
        try:
            ...
            dirname_ = filename.replace(".zip", "") + f"_v{get_server_version()[0]}"

            # 创建文件夹，并解压
            os.mkdir(dirname_)
            file.extractall(dirname_)
            file.close()
            # 递归修复编码
            self.rename(dirname_)
            return dirname_
